...
```bash
docker ps # list only runnig container
...
```bash
docker rm -f 2a # without stopping remove the container 
docker ps -a
```
```bash
...
- Run an another `ubuntu` container with interactive shell open, name the container as `babak4`, attach the volume `my-vol` as read-only to `/test4` mount point in the container, and add command to run bash shell.

```bash
docker run -it --name babak4 -v my-vol:/test4:ro ubuntu bash
```
...
- Try to create an another file under `/test4` folder. Should see error `read-only file system`

```bash
cd test4 && touch file-from-4th.txt  # ro !!!
...
- Delete `babak2`, `babak3` and `babak4` containers.

```bash
docker rm babak2 babak3 babak4
```
...
- Run an `alpine` container with interactive shell open, name the container as `vol-lesson`, attach the volume `full-vol` to `/test` mount point in the container, and add command to run alpine shell. 
...
```bash
cd test && echo "This file is created in the full-vol volume" > full.txt #rw become direct in the container
```
...
sudo cat /var/lib/docker/volumes/full-vol/_data/full.txt
```

- Run the `ondiacademy/hello-ondia` container with interactive shell open, name the container as `babak`, and show the inside of `hello-ondia` directory.

```bash
docker run -it --name babak ondiacademy/hello-ondia sh
/ # ls
...
- Run the `ondiacademy/hello-ondia` container with interactive shell open, name the container as `try1`, attach the volume `full-vol` to `/test` mount point in the container, and show that `/test` directory is created and files inside volume are copied to this directory.

```bash
docker run -it --name try1 -v full-vol:/test ondiacademy/hello-ondia sh
/ # ls
bin           dev           hello-ondia  lib           mnt           proc          run           srv           tmp           var
test            etc           home          media         opt           root          sbin          sys           usr
/ # cd test && ls
full.txt
# cat full.txt
```
...
- Run the `ondiacademy/hello-ondia` container with interactive shell open, name the container as `try2`, attach the volume `empty-vol` to `/hello-ondia` mount point in the container.
...
- Run the `ondiacademy/hello-ondia` container with interactive shell open, name the container as `try3`, attach the volume `full-vol` to `/hello-ondia` mount point in the container, and show that we just see the files inside volume regardless of  the target directory is full or empty.
...
docker container ls
```
## You can attach the container multiple volumes
```bash
...
<h1>Welcome to Docker Lesson</h1>
```

- `exit` the container.

- Add `<h2>This is added for docker volume lesson</h2>` line to index.html in the /home/ec2-user/webpage folder and 
...
```bash
docker run -dit -v /data --name cont1 busybox sh # with sh container stay up
```
...
docker ps -a
docker start 5ac
docker exec -it 5 touch file1
docker exec -it 5 ls file1

docker attach → Used to connect to an existing process of a running container.

docker attach 5 __> attantion !! when you exit then the container prosess finished so that container also stopped