...
- Note that while the size of `babaktanriverdi/flask-app:1.0` is approximately 473MB, the size of `babaktanriverdi/flask-app:2.0` is 67MB.

- Run the newly built image as a container in detached mode, connect host's `port 8080` to container's `port 80`, and name the container as `welcome2`. Then list running containers and connect to the EC2 instance from the browser to show the Flask app is running.
...
- Stop and remove the container `welcome2`.
