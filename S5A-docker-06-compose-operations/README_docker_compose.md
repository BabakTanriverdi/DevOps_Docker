...
- Check if the `Docker Compose` is working. Should see something like `Docker Compose version v2.40.0`
...
- `COPY app.py .` same as `COPY . .`
- `CMD ["flask", "run"]` → exec form (Handson-10)
...
- `CMD ["flask", "run"]` → Best practice. Uses Flask's own command, libraries and ENV.
- `CMD ["python3", "./app.py"]` → Also works but you must delete `ENV FLASK_APP app.py`.
...
services:
  web:
    build: .          # When your Dockerfile is in a subdirectory: "build: ./app"
    ports:
      - "5000:5000"   # Expose port to reach the container from outside
  redis:
...
- When you have a different name for Dockerfile you must write `context: ./app` and `file: my_dockerfile`
...
- Open aNOTEher terminal and connect to the EC2 instance.
...
- Run the `docker compose ps` command. NOTEice that it will NOTE work, producing an error. Mention that docker compose commands must be implemented in the related directory.
...
docker ps -a
docker network ls
docker network inspect 06d97c723db6
```
...
**NOTE** `docker compose down` removes services and network but NOTE volumes and images.
...
- Run the `docker images` command to see if images are NOTE removed. NOTEice that images and volumes are NOTE removed with the `docker compose down` command.
